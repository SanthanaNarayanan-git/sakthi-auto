...
exports.getMonthlyReport = async (req, res) => {
  try {
    ...
    // 🔥 Fetches SupervisorSignature for the NCR table in PDF
...
exports.getReportsBySupervisor = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: "Failed to fetch supervisor reports" }); }
};
...
exports.signReportBySupervisor = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: "Failed to save supervisor signature" }); }
};

// 🔥 Fetch NCRs for Supervisor Dashboard
exports.getNcrReportsBySupervisor = async (req, res) => {
  try {
    ...
    `;
    res.json(result.recordset);
  } catch (error) { res.status(500).json({ error: "Failed to fetch NCRs" }); }
};

// 🔥 Signs specific NCR and automatically sets Status = 'Completed'
exports.signNcrBySupervisor = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: "Failed to save NCR signature" }); }
};
