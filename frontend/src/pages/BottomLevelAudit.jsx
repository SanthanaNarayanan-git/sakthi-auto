...
const BottomLevelAudit = () => {
  ...
  const handleMasterHolidayToggle = (checked) => { setChecklist(prev => prev.map(c => ({ ...c, IsHoliday: checked, IsVatCleaning: false, IsDone: false, IsNA: false }))); };
  const handleMasterVatToggle = (checked) => { setChecklist(prev => prev.map(c => ({ ...c, IsVatCleaning: checked, IsHoliday: false, IsDone: false, IsNA: false }))); };
  ...
  const handleBatchSubmit = async () => {
    ...
    setLoading(true);
    try {
      const itemsToSave = checklist.map(item => ({ MasterId: item.MasterId, IsDone: item.IsDone, IsNA: item.IsNA, IsHoliday: item.IsHoliday, IsVatCleaning: item.IsVatCleaning }));
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      try {
          ...
          // 🔥 FIX: Stops execution immediately if the backend 500 error occurs
          console.error("Backend Error:", backendErr);
          setNotification({ show: true, type: 'error', message: 'Database Error: Missing SQL columns. Check backend console.' });
          return; 
      }
      ...
      // 🔥 FIX: Removed optional chaining ?. to prevent older React environments from breaking
      ...
      monthlyLogs.forEach(log => {
        ...
        if (log.IsNA == 1) { historyMap[key][logDay] = 'NA'; } 
        else if (log.IsDone == 1) { historyMap[key][logDay] = 'Y'; } 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-xs text-gray-500 uppercase border-b-2 border-blue-100">
                ...
                <th className="py-3 text-center w-20 ">Holiday<br/><input type="checkbox" checked={isGlobalHoliday} onChange={(e) => handleMasterHolidayToggle(e.target.checked)} className="w-4 h-4 mt-6 accent-orange-600 cursor-pointer"/></th>
            ...
            <tbody>
              {checklist.length === 0 ? <tr><td colSpan="7" className="text-center py-4 text-gray-500">Loading...</td></tr> : checklist.map((item) => {
                const hasReport = !!reportsMap[item.MasterId]; const isDisabled = item.IsHoliday || item.IsVatCleaning;
                return (
        ...
        <div id="checklist-footer" className="bg-slate-100 p-8 border-t border-gray-200 flex justify-between gap-6 rounded-b-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
           <div className="flex flex-col gap-4 w-full md:w-1/2">
             <div className="w-full md:w-3/4">
                 <label className="text-[11px] font-black text-gray-600 uppercase block mb-1">Assign Supervisor {(isGlobalHoliday || isGlobalVatCleaning) && <span className="text-gray-400 lowercase font-normal">(Optional)</span>}</label>
                 <SearchableSelect options={supervisors} displayKey="OperatorName" value={headerData.supervisorName} onSelect={(op) => setHeaderData(prev => ({...prev, supervisorName: op.OperatorName}))} placeholder={isGlobalHoliday || isGlobalVatCleaning ? "Not Required" : "Select Supervisor..."} />
             </div>
             <div className="w-full md:w-3/4">
               <label className="text-[11px] font-black text-gray-600 uppercase block mb-1">Assign to HOF (Month End) {(isGlobalHoliday || isGlobalVatCleaning) && <span className="text-gray-400 lowercase font-normal">(Optional)</span>}</label>
               <SearchableSelect options={hofs} displayKey="OperatorName" value={headerData.assignedHOF} onSelect={(op) => setHeaderData(prev => ({...prev, assignedHOF: op.OperatorName}))} placeholder={isGlobalHoliday || isGlobalVatCleaning ? "Not Required" : "Select HOF..."} />
             </div>
           </div>
